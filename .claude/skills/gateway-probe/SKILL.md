...
```bash
uv run python .claude/skills/gateway-probe/probe.py [options]
```
...
uv run python .claude/skills/gateway-probe/probe.py --listen-on-connect 3 --listen 3
```
...
- This is debug tooling under `.claude/`, not shipped product code — keep
