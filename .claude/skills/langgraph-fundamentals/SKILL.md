...
For advanced topics, see these dedicated skills:

- **langgraph-multi-agent** — Supervisor, swarm, hierarchical teams, agent handoffs
- **langgraph-streaming** — All stream modes, subgraph streaming, custom events
- **langgraph-configuration** — Runtime config, context_schema, dynamic model/tool selection
- **langgraph-human-in-the-loop** — Interrupt, resume, approval workflows
- **langgraph-persistence** — Checkpointers, time travel, Store
- **langgraph-trajectory** — Execution trace recording, RL export, replay from failure
- **langgraph-debugging** — Studio, LangSmith, graph visualization, XRay
</related-skills>
...
- **"How does the Pregel execution engine process supersteps?"** — core execution model
- **"What are all the channel types (LastValue, Topic, BinaryOperatorAggregate, EphemeralValue)?"** — advanced state management
- **"How does add_conditional_edges work internally with the router function?"** — conditional routing details
- **"How does the Command object combine state updates with routing?"** — Command internals
- **"How does Send create parallel worker instances?"** — fan-out mechanics
- **"What is the recursion_limit and how to configure it?"** — preventing runaway graphs
