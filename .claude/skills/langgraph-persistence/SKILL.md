...
Resume execution from the exact failure point using checkpointed state.

<ex-failure-recovery>
<python>
Detect and recover from execution failures using checkpoints.
```python
from langgraph.checkpoint.memory import InMemorySaver

checkpointer = InMemorySaver()
config = {"configurable": {"thread_id": "recovery-task"}}
...
print(f"Stopped at: {state.next}")         # Node that needs to re-run
...
# Strategy 1: Simple retry (re-run failed node with same state)
result = graph.invoke(None, config)

# Strategy 2: Fix state then retry
graph.update_state(config, {
    "messages": state.values["messages"] + ["[System] Retry with fixed parameters"],
})
result = graph.invoke(None, config)

# Strategy 3: Roll back to a known-good checkpoint
history = list(graph.get_state_history(config))
good = next(
    s for s in history
    if not any(hasattr(t, "error") and t.error for t in (s.tasks or []))
    and s.metadata.get("source") == "loop"
)
result = graph.invoke(None, good.config)
```
</python>
</ex-failure-recovery>
...
Fork execution from any historical checkpoint for debugging.

<ex-debug-fork>
<python>
Fork from a specific checkpoint to test alternative execution paths.
```python
config = {"configurable": {"thread_id": "debug-run"}}

# Complete a full execution
result = graph.invoke({"messages": ["Full task"]}, config)
...
for i, snap in enumerate(history):
    ...
    print(f"[{i}] Step {step} ({source}): next={snap.next}")

# Fork from a past checkpoint to try a different approach
target = history[3]
fork_config = graph.update_state(
    target.config,
    {"messages": ["[Debug] Alternative approach..."]},
)

# New execution branch from the fork point
...
- **"How does PostgresSaver handle concurrent writes and connection pooling?"** — production checkpointer details
- **"How does checkpoint_ns scope subgraph checkpoints?"** — namespace isolation internals
- **"How to use Overwrite with update_state to bypass reducers?"** — state replacement
- **"How does the Store search() method work with filters?"** — store query patterns
- **"How does get_state_history order checkpoints and track parent_config?"** — checkpoint chain
- **"How to access subgraph state separately from parent graph state?"** — nested state inspection
