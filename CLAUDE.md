...
- **Home contrib**: `~/.agentm/contrib/extensions/<name>.py` and
  ...
  Claude Code plugins). Respects `$AGENTM_HOME` override. Use
...
| Code health | `uv run agentm lint src/agentm/` | Project-specific AST checks (AM001–AM012): silent exceptions, missing slots, atom raw I/O, config-dict-splat, etc. AM001 errors must stay at zero. |
