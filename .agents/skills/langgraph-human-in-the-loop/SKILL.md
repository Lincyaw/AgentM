...
## Supervisor Interrupt Patterns

When a supervisor needs to monitor and intervene in sub-agent execution, combine `interrupt()` with the multi-agent patterns.

<ex-supervisor-interrupt>
<python>
Sub-agent pauses at checkpoints for supervisor review.
```python
from langgraph.types import interrupt, Command
...
def worker_with_checkpoints(state: MessagesState) -> dict:
    """Worker that pauses for supervisor review at key milestones."""
    # Phase 1: Do initial analysis
    analysis = perform_analysis(state["messages"])

    # Checkpoint 1: Let supervisor review analysis before proceeding
    supervisor_input = interrupt({
        "phase": "analysis_complete",
        "result": analysis,
        "options": ["continue", "revise", "abort"],
    })

    if supervisor_input["decision"] == "abort":
        return {"messages": ["[Worker] Aborted by supervisor"]}
    if supervisor_input["decision"] == "revise":
        analysis = revise_analysis(analysis, supervisor_input.get("feedback", ""))

    # Phase 2: Implementation
    implementation = implement(analysis)

    # Checkpoint 2: Final review
    final_review = interrupt({
        "phase": "implementation_complete",
        "result": implementation,
        "options": ["approve", "reject"],
    })

    if final_review["decision"] == "reject":
        return {"messages": ["[Worker] Implementation rejected"]}

    return {"messages": [f"[Worker] Complete: {implementation}"]}

graph = (
    StateGraph(MessagesState)
    .add_node("worker", worker_with_checkpoints)
    .add_edge(START, "worker")
    .add_edge("worker", END)
    .compile(checkpointer=InMemorySaver())
)

config = {"configurable": {"thread_id": "supervised-task"}}

# Start — worker pauses at checkpoint 1
result = graph.invoke({"messages": ["Build feature X"]}, config)
# result["__interrupt__"] shows analysis for review
...
# Worker continues to checkpoint 2

# Supervisor approves final result
result = graph.invoke(
...
<ex-dynamic-instruction-injection>
<python>
Supervisor injects new instructions into a paused sub-agent.
```python
config = {"configurable": {"thread_id": "dynamic-task"}}

# Worker is paused at an interrupt
state = graph.get_state(config)

# Supervisor injects new context before resuming
graph.update_state(config, {
    "messages": [
        *state.values["messages"],
        "[Supervisor] Additional context: prioritize error handling",
    ],
})

# Resume with new instructions in state
result = graph.invoke(
    Command(resume={"decision": "continue"}),
    config,
)
...
- **"How does interrupt() interact with subgraph checkpointer scoping?"** — interrupts in nested graphs
- **"How to use interrupt_before and interrupt_after compile options?"** — declarative breakpoints
- **"How does the Interrupt object store the interrupt ID for multiple parallel interrupts?"** — parallel interrupt mechanics
- **"How to use Command(resume=...) with subgraphs that have their own interrupts?"** — nested resume
- **"How does interrupt() handle non-JSON-serializable values?"** — serialization constraints
