...
def tool_call_response(
    ...
    return AssistantMessage(
        role="assistant",
        content=[
            ToolCallBlock(type="tool_call", id=call_id, name=name, arguments=args)
        ],
        timestamp=time.time(),
...
def make_model() -> Model:
    return Model(
        id="mock-model", provider="mock", context_window=128000, max_output_tokens=4096
    )
...
async def test_cross_atom_services() -> None:
    ...
    async def increment_fn(args: dict[str, Any]) -> ToolResult:
        counter["value"] += 1
        return ToolResult(
            content=[TextContent(type="text", text=f"counter={counter['value']}")]
        )

    async def report_fn(args: dict[str, Any]) -> ToolResult:
        return ToolResult(
            content=[TextContent(type="text", text=f"report: {counter['value']}")]
        )
...
async def test_context_policy() -> None:
    ...
    session.bus.on(
        BeforeSendEvent.CHANNEL, lambda ev: captured_messages.extend(ev.messages)
    )
...
def test_trajectory_query_store_adapter_filters_sessions_and_turns() -> None:
    ...
    assert [
        session.id for session in query.sessions(SessionFilter(root_session_id="root"))
    ] == ["root", "child"]
    assert [
        session.id for session in query.sessions(SessionFilter(purpose="worker"))
    ] == ["child"]
    assert [
        session.id for session in query.sessions(SessionFilter(since=15.0, limit=1))
    ] == ["child"]
...
async def test_stream_fn_exception_persists_non_replayable_turn() -> None:
    ...
    session = Session(
        SessionRuntimeConfig(  # type: ignore[arg-type]
            stream_fn=failing,
            model=make_model(),
            system="test",
        )
