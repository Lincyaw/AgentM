...
@pytest.mark.asyncio
async def test_stop_counter_resets_after_successful_activate(
    tmp_path: Path,
) -> None:
    """Two rejected entries followed by a successful ``activate`` resets
    the chain. A subsequent rejected entry counts as 1, not 3 — the gate
    must let an activate through. Without this the counter is monotonic
    and every tuner is eventually permanently blocked.
    """
    decisions_dir = tmp_path / ".agentm" / "decisions" / "format_fix"
    _seed_rejections(decisions_dir, count=2)
    _seed_activate(decisions_dir)
    _seed_rejections(decisions_dir, count=1)

    # With threshold=3, only 1 consecutive rejection sits at the tail —
    # we need a successful path through the gate to confirm reset works.
    eval_runs = tmp_path / ".agentm" / "eval_runs"
    eval_runs.mkdir(parents=True, exist_ok=True)
    ...
    # Materialize a tiny target atom on disk so cross-session resolution
    # finds it.
    scenario_dir = tmp_path / "contrib" / "scenarios" / "format_fix"
    scenario_dir.mkdir(parents=True)
    (scenario_dir / "tool_x.py").write_text(
        ...
        result = await propose.execute(
            {
                "target": {
                    ...
                    "new_content": (
                        "from agentm.core.abi import FunctionTool, TextContent, ToolResult\n"
                        "from agentm.extensions import ExtensionManifest\n"
                        "MANIFEST = ExtensionManifest(name='tool_x',"
                        " description='x', registers=('tool:x',))\n"
                        "async def _exec(args):\n"
                        "    return ToolResult(content=[TextContent(type='text', text='v2')])\n"
                        "def install(api, config):\n"
                        "    api.register_tool(FunctionTool(name='x',"
                        " description='x', parameters={'type':'object',"
                        "'properties':{}}, fn=_exec))\n"
                    ),
                    "target_atom": "tool_x",
                },
                "rationale": "post-reset activate",
                "eval_run_baseline": "er_b",
                "eval_run_proposed": "er_p",
                "decision": "activate",
            }
        )
        # Counter reset by intervening activate -> only 1 trailing reject.
        # The gate should NOT block on stop_after_no_improvement.
        assert (
            result.is_error is False
            or "stop_after_no_improvement" not in result.content[0].text
