...
@pytest.mark.asyncio
async def test_merge_passing_all_floors_activates(tmp_path: Path) -> None:
    """Complement: a merge child whose delta clears threshold + 2σ +
    guard floors does activate. This pins down the contract from the
    other side — the gate is not just a "merge always rejects" rubber
    stamp.

    Scores: baseline 0.10 ± 0.05, proposed 0.95 ± 0.05.
    - relative = 8.5 >> 0.05 PASSES
    - 2·sqrt(0.0025 + 0.0025) = 2·0.0707 ≈ 0.141; delta 0.85 >> 0.141 PASSES
    - guard floors empty so trivially pass.
    """
    scenario_dir, decisions_dir = _setup_repo(tmp_path)
    _write_eval_run(
        tmp_path, "er_b", primary_score=0.10, primary_score_stderr=0.05
    )
    _write_eval_run(
        tmp_path, "er_p", primary_score=0.95, primary_score_stderr=0.05
    )
    ...
    try:
        ...
        result = await propose.execute(
            {
                ...
                "rationale": "merge of c_parent_a + c_parent_b: real win",
                "parents": ["c_parent_a", "c_parent_b"],
                "eval_run_baseline": "er_b",
                "eval_run_proposed": "er_p",
                "decision": "merge",
        ...
        assert payload["status"] == "merge"
        # Atom-on-disk swapped to v2.
        assert (
            scenario_dir / "tool_x.py"
        ).read_text(encoding="utf-8") == _ATOM_SRC_V2

        # Activation record has kind=merge with parent_ids preserved.
        records = [
            json.loads(line)
            for line in (decisions_dir / "activations.jsonl")
            .read_text(encoding="utf-8")
        ...
        merge_recs = [r for r in records if r["kind"] == "merge"]
        assert merge_recs
        assert set(merge_recs[-1]["parent_ids"]) == {
            "c_parent_a",
            "c_parent_b",
        }

        # Candidate companion record carries parent_ids (the schema
        # migration). Find the new c_* candidate (the parents are
        # c_parent_a/c_parent_b — the merge child has a c_<hash> id).
        candidates_dir = decisions_dir / "candidates"
        merge_child_files = [
            p
            for p in candidates_dir.glob("c_*.json")
            if p.stem not in ("c_parent_a", "c_parent_b")
        ]
        assert len(merge_child_files) == 1
        with merge_child_files[0].open("r", encoding="utf-8") as fh:
            child_rec = json.load(fh)
        assert set(child_rec["parent_ids"]) == {"c_parent_a", "c_parent_b"}
    finally:
        await session.shutdown()


@pytest.mark.asyncio
async def test_merge_requires_two_parents(tmp_path: Path) -> None:
    """``decision='merge'`` with <2 parents is rejected before any gate
    work. Sanity check on the input contract — without it the merge
    channel collapses to a re-skinned ``activate``."""
    ...
    try:
        ...
        result = await propose.execute(
            {
                ...
                "rationale": "bogus single-parent merge",
                "parents": ["c_parent_a"],
                "eval_run_baseline": "er_b",
                "eval_run_proposed": "er_p",
                "decision": "merge",
        ...
        assert ">=2" in result.content[0].text or "evidence missing" in result.content[0].text

        # Unknown parent — must reject.
        result2 = await propose.execute(
            {
                ...
                "rationale": "bogus unknown-parent merge",
                "parents": ["c_parent_a", "c_does_not_exist"],
                "eval_run_baseline": "er_b",
                "eval_run_proposed": "er_p",
                "decision": "merge",
            }
        ...
        assert "unknown parent candidate" in result2.content[0].text
    finally:
        await session.shutdown()
