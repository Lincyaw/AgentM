...
@pytest.mark.asyncio
async def test_pareto_dominated_candidate_excluded(tmp_path: Path) -> None:
    """Sanity: a candidate that is strictly dominated (loses on every
    task by a non-zero margin) is NOT on the frontier. This is the
    *complement* of the niche-winner test — together they pin down the
    inclusion criterion."""
    scenario = "pareto_demo2"
    candidates_dir = (
        tmp_path / ".agentm" / "decisions" / scenario / "candidates"
    )
    _write_candidate(
        candidates_dir,
        candidate_id="c_winner",
        per_task_scores={"t1": 0.90, "t2": 0.90},
    )
    _write_candidate(
        candidates_dir,
        candidate_id="c_loser",
        per_task_scores={"t1": 0.10, "t2": 0.10},
    )
    ...
    try:
        ...
        result = await tool.execute({})
        payload = json.loads(result.content[0].text)
        frontier_ids = {e["candidate_id"] for e in payload["frontier"]}
        assert "c_winner" in frontier_ids
        assert "c_loser" not in frontier_ids
        assert "c_loser" in payload["dominated"]
    finally:
        await session.shutdown()
