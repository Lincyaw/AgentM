...
from agentm.ai.api_registry import clear_api_providers, get_api_provider, register_api_provider
...
class _Provider:
    ...
    async def stream(self, model: Model, context: Any, options: Any | None = None) -> Iterable[Any]:
...
def test_env_key_resolution_uses_descriptor_precedence_with_in_memory_env() -> None:
    ...
    assert find_env_keys("anthropic", env) == ["ANTHROPIC_OAUTH_TOKEN", "ANTHROPIC_API_KEY"]
...
def test_default_provider_metadata_covers_builtin_vendors() -> None:
    ...
    assert tuple(dict.fromkeys(d.api for d in DEFAULT_PROVIDER_REGISTRY.descriptors())) == KNOWN_APIS
...
def test_registering_new_vendor_only_requires_descriptor_and_factory() -> None:
    ...
    assert registry.get_env_api_key("fake-provider", {"FAKE_API_KEY": "secret"}) == "secret"
...
async def test_validating_provider_uses_canonical_api_for_stream_methods() -> None:
    ...
    with pytest.raises(ValueError, match="Mismatched api"):
        await provider.stream(Model(id="claude", provider="anthropic", api="anthropic-messages"), "ctx")
