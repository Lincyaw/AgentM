"""High-value regression tests for CriticSanitizer."""
...
async def test_c3_is_skipped_when_no_confirmed_hypothesis() -> None:
    sanitizer = CriticSanitizer(MockModel(CausalDirectionResult(proven=False, missing_evidence="unused")))
...
async def test_j1_warns_on_misread_rejection_with_sufficient_evidence() -> None:
    sanitizer = CriticSanitizer(MockModel(SignalMisreadResult(misread=True, reasoning="evidence was strong")))
...
async def test_p2_async_check_reports_dispatch_quality_issue() -> None:
    sanitizer = CriticSanitizer(MockModel(DispatchQualityResult(quality="poor", missing=["service names", "metrics"])))

    tracker = InvestigationTracker()
    tracker.record(1, "dispatch", {"instruction": "go investigate stuff"})

    await sanitizer.check_async("dispatch", HypothesisStore(), ServiceProfileStore(), tracker, _make_ctx())
...
async def test_config_controls_severity_and_disabled_checks() -> None:
    sanitizer = CriticSanitizer(
        MockModel(
            {
                CausalDirectionResult: CausalDirectionResult(proven=False, missing_evidence="would block"),
                SignalMisreadResult: SignalMisreadResult(misread=False),
                SymptomAsCauseResult: SymptomAsCauseResult(likely_symptom=True, suspect_upstream=["db-svc"]),
            }
        ),
