...
// It includes all built-in themes (including "default") and user themes from ThemesDir().
...
// listUserThemeRefs returns the list of user theme references from ThemesDir().
...
// in the user themes directory (typically $AGENTM_HOME/terminal-go/data/themes).
//
// This handles the "user:" prefix - "user:nord" checks for ThemesDir()/nord.yaml.
