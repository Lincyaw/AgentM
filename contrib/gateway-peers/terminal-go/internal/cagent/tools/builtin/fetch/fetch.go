...
const (
	...
	// defaultToolHTTPTimeout mirrors the upstream httpclient default. The fetch
	// tool never executes on the wire-render path, so the SSRF-safe transport
	// and JS header expansion are dropped; only this static default remains.
	defaultToolHTTPTimeout = 30 * time.Second

	// userAgentHeader is the User-Agent value attached to outbound fetch
	// requests. It doubles as the agent name fed to robots.txt matching, so
	// site operators see one identity in both places. The upstream variant
	// embedded a build version and a Docker Desktop probe; neither is
	// available on the wire-render client, so a static identity is used.
	userAgentHeader = "Cagent/dev"

	// headerAgentVersion mirrors the X-Docker-Agent-Version header the upstream
	// tools stamp; the client has no build version, so a static value is sent.
	headerAgentVersion = "X-Docker-Agent-Version"
)

// setIdentity stamps the request with the User-Agent and agent-version
// headers. The upstream variant also added an X-Docker-Desktop-Version header
// sourced from a Desktop probe, which never runs on the wire-render path.
func setIdentity(req *http.Request) {
	req.Header.Set("User-Agent", userAgentHeader)
	req.Header.Set(headerAgentVersion, "dev")
}
...
func (h *fetchHandler) CallTool(ctx context.Context, params ToolArgs) (*tools.ToolCallResult, error) {
	...
	// The SSRF-safe transport never executes on the wire-render path, so the
	// client uses the stdlib default transport directly. The allowPrivateIPs
	// knob is preserved for config-shape compatibility but no longer selects a
	// hardened dialer.
	var transport http.RoundTripper = http.DefaultTransport

	// JS template-literal expansion of headers is a backend behaviour that does
	// not run client-side; headers are used verbatim.
	headers := h.headers
...
func (h *fetchHandler) fetchURL(ctx context.Context, client *http.Client, urlStr, format string, headers map[string]string, robotsCache map[string]*robotstxt.RobotsData) Result {
	...
	if robots != nil && !robots.TestAgent(parsedURL.Path, userAgentHeader) {
...
func New(options ...ToolOption) *ToolSet {
	tool := &ToolSet{
		handler: &fetchHandler{
			timeout: defaultToolHTTPTimeout,
		},
	}
