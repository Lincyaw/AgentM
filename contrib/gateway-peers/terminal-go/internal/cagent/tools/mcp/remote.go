...
// headerTransport stamps a fixed set of HTTP headers onto every request. The
// upstream variant resolves ${headers.NAME} placeholders from per-request
// upstream context; that template expansion is backend behaviour that never
// runs on the wire-render path, so headers are applied verbatim here.
...
// that do not support Dynamic Client Registration (RFC 7591). It mirrors the
// handful of fields the OAuth flow reads; the full config schema is loaded
// server-side and never reaches this client on the wire path.
...
func (c *remoteMCPClient) headerTransport() http.RoundTripper {
	if len(c.headers) > 0 {
		return &headerTransport{base: http.DefaultTransport, headers: c.headers}
	}
