"""Offline audit pipeline for recorded AgentM trajectories.

Runs the llmharness auditor (and optionally the trajectory-index extractor)
over a recorded message sequence and returns surfaced reminders. This is
eval-only infrastructure; the runtime atom uses a different code path.
"""
...
from llmharness.offline import AuditorSettings, flatten_assistant_blocks, run_phase_standalone
from llmharness.schema import Reminder, Verdict
from llmharness.state import CumulativeAuditState
from llmharness.trajectory_utils import extract_loaded_skills, serialize_trajectory
...
async def audit_pipeline_over_trajectory(
    *,
    ...
    for turn_number, prefix_len in enumerate(_turn_end_prefix_lengths(messages), start=1):
        ...
        if auditor_due:
            trajectory = serialize_trajectory(prefix)
            ...
            loaded_skills = extract_loaded_skills(prefix)
            prompt_text = (
