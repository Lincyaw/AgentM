"""``judge_coverage`` — ``rca.judge.coverage`` service atom.

Phase 2 C1 of the rca_hfsm scenario. Registers a single ``Judge``
implementation under the service name ``rca.judge.coverage`` and
toggles between an LLM-backed and a scripted (stub) backing
implementation via ``config.mode`` (default: ``"llm"``).

Replaces the Phase 1 mechanical symptom-chain walk in
``rca_hgraph_store.get_unexplained_symptoms`` /
``rca_falsification_gate._apply_confirm`` (design §4.2). C1 only mounts
the judge as a service; the gate continues to use its Phase 1 rules.
The gate refactor is C2's job.

JudgeContext shape: ``graph_slice = {"hypothesis", "predictions",
"symptoms", "observations"}`` with ``operands = {"hypothesis_id":
"<H>"}``. Canonical verdict strings per design §4.2: ``"covers" |
...
from agentm_rca_hfsm.judges import (
    JudgeContext,
...
_PROMPT_RELPATH = f"contrib/scenarios/rca_hfsm/prompts/judges/{_KIND}.md"
_LRU_MAX = 256
...
MANIFEST = ExtensionManifest(
    name="judge_coverage",
    description=(
        "Registers the rca.judge.coverage service. LLM-backed by default; "
...
def _load_prompt(cwd: str) -> str:
    for candidate in (
        ...
        Path(__file__).resolve().parents[3] / "prompts" / "judges" / f"{_KIND}.md",
    ):
        if candidate.exists():
