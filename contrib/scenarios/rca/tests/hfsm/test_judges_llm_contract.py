"""LLM-mode contract tests for the 4 judge atoms.

For each judge we install with ``mode="llm"`` and a fake provider whose
``stream_fn`` returns a single ``MessageEnd`` carrying an
``AssistantMessage`` with a ``ToolCallBlock`` for ``submit_verdict``.

Three cases per judge:
...
# Workspace root — six levels up from this test file:
#   parents[0] hfsm
#   parents[1] tests
#   parents[2] rca
#   parents[3] scenarios
#   parents[4] contrib
#   parents[5] <workspace root>
...
class _FakeStreamFn:
    ...
    * a ``dict`` with verdict/reason/confidence keys → emit one
      ``MessageEnd`` carrying a ``submit_verdict`` ``ToolCallBlock``;
    * the string ``"malformed"`` → emit a ``MessageEnd`` whose
      assistant message contains only a ``TextContent`` block (so
      ``_parse_submit_verdict`` raises ``ValueError``);
    * an instance of ``Exception`` → raise it instead of yielding.

    Honours the same kwargs as the real ``StreamFn`` Protocol.
    """
...
class _LlmAPI:
    """Minimal ExtensionAPI shim covering the surface judge atoms touch."""
...
def test_llm_happy_path_returns_payload(module: Any, service_name: str) -> None:
    stream = _FakeStreamFn(
        [{"verdict": "satisfied", "reason": "ok", "confidence": "high"}]
    )
...
@pytest.mark.parametrize(("module", "service_name"), _JUDGES)
def test_llm_retries_once_on_malformed(module: Any, service_name: str) -> None:
    stream = _FakeStreamFn(
        [
            "malformed",
            {"verdict": "satisfied", "reason": "after retry", "confidence": "medium"},
        ]
    )
    api = _LlmAPI(stream_fn=stream)
    module.install(api, {"mode": "llm"})
...
@pytest.mark.parametrize(("module", "service_name"), _JUDGES)
def test_llm_falls_back_to_unclear_on_double_failure(
