...
def test_manifest_loads_in_declared_order() -> None:
    ...
    try:
        extensions, _meta = load_scenario_with_meta("rca:hfsm")
    finally:
        if prior is None:
    ...
    # The whole stack present (17 entries — see manifest.hfsm.yaml).
    # 11 hfsm atoms + 5 LLM-native judge atoms
    # + 1 data-access atom (duckdb_sql — design §13 reuse)
    assert len(modules) == 17, modules
    # Critical dependency-order properties:
    assert modules.index(
        "agentm_rca.hfsm.atoms.rca_hgraph_store"
    ) < modules.index("agentm_rca.hfsm.atoms.rca_falsification_gate")
    assert modules.index(
        "agentm_rca.hfsm.atoms.rca_falsification_gate"
    ) < modules.index("agentm_rca.hfsm.atoms.rca_evidence_tools")
    ...
    for kind in (
        ...
        judge_module = f"agentm_rca.hfsm.atoms.judge_{kind}"
