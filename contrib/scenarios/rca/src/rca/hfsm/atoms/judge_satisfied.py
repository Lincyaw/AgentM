...
class JudgeSatisfiedConfig(BaseModel):
    mode: str = "llm"
    model: str | None = None
...
MANIFEST = ExtensionManifest(
    ...
    registers=(),
    config_schema=JudgeSatisfiedConfig,
    requires=(),
)
    ...
    async def _judge_async(self, context: JudgeContext) -> Verdict:
        ...
        for attempt in range(2):
            try:
                ...
                logger.debug("judge_satisfied attempt {} failed: {}", attempt, exc)
...
def install(api: ExtensionAPI, config: JudgeSatisfiedConfig) -> None:
