"""Ingest ops-lite-clean into the rca llm-eval DB.

Walks ``datasets/ops-lite-clean/cases/*`` and inserts a ``DatasetSample``
row per case under ``dataset='ops-lite-clean'``. Idempotent.

Run:
    uv run python contrib/scenarios/rca/eval/baseline/ingest_ops_lite_clean.py
"""
...
DATASET_NAME = "ops-lite-clean"
CASES_DIR = Path("datasets/ops-lite-clean/cases").resolve()
...
def main() -> None:
    try:
        ...
        raise SystemExit(
            f"needs rcabench-platform + sqlmodel. Original error: {exc}"
        ) from exc
