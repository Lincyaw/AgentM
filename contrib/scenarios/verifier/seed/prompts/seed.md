...
- For URL/path `JVMRuntimeMutator` faults, do not treat zero abnormal attempts as automatic "method not exercised". If the affected target endpoint or method has normal-window spans, its caller-owned endpoint has normal-window spans, the abnormal window shows a selective disappearance of that same path on both target and caller, the target service is still alive from metrics/logs/other spans, and the disappearance is not merely proportional global traffic collapse, this is the mutation's expected flow-interruption signature. Confirm with predicate `flow_interrupted` even when there are no abnormal child spans to inspect. Use `inconclusive` only when you cannot establish the normal affected path, cannot show the target remained alive, or cannot distinguish the disappearance from broad workload/source absence.
