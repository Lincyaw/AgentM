...
from injection import _load_fault_doc, get_injections, get_target_evidence
from verdict import collect_all_verdicts

REPO = Path(__file__).resolve().parents[4]
WORKFLOW_SCRIPT = Path(__file__).resolve().parent / "propagation_workflow.py"
JUDGE_PROMPT_PATH = REPO / "contrib" / "scenarios" / "verifier_judge" / "prompt.py"
...
_judge_prompt_module = None


def _get_judge_prompt_module():  # type: ignore[return]
    import importlib.util
    global _judge_prompt_module  # noqa: PLW0603
    if _judge_prompt_module is not None:
        return _judge_prompt_module
    spec = importlib.util.spec_from_file_location(
        "verifier_judge.prompt", JUDGE_PROMPT_PATH,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"cannot load {JUDGE_PROMPT_PATH}")
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    _judge_prompt_module = mod
    return mod


def _extract_judge_verdict(messages: list) -> dict | None:  # type: ignore[type-arg]
...
async def _run_judge_async(
    ...
    try:
        ...
        return _extract_judge_verdict(messages)
    finally:
        await session.shutdown()
