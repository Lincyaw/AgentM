...
| **Extractor child** | `audit/extractor/` | Phase 1. Per-turn LLM-driven graph builder. Single terminal tool `submit_events` emits typed events with embedded `refs[]`; the witness layer validates entities + verbatim quotes against the source turns and unrolls refs into `Edge` records. |
| **Auditor child** | `audit/auditor/` | Phase 2. Every k turns. Reads the live graph (events + edges + scenario findings + recent verdicts + continuation_notes from the prior firing), plus a `get_event_detail` / `get_turn` drill-down tool above the summary threshold. Terminal tool: `submit_verdict`. |
