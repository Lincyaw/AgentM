...
**Counterfactual test.** Before flagging: if this value were wrong, would the final answer change? If not, it is not load-bearing. If yes, find where the answering agent first committed to this value as settled fact — not a passthrough of a tool result, not where a sub-agent proposed it, and not where the agent merely speculated. That commitment is the decisive step.
