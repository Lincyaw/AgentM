...
``act`` / ``evid`` events the auditor must reason over. The phase
merger collapses consecutive ``act`` / ``evid`` runs into a single
"basic block"-style :class:`~llmharness.schema.Phase` so the auditor
sees high-level investigation phases instead of every tool call.

Boundaries:

- ``task`` / ``hyp`` / ``dec`` / ``concl`` always become singleton
  phases. They are decision / framing points and never merge.
- A maximal run of consecutive ``act`` / ``evid`` events between two
  break-kind events becomes one merged phase with kind
  ``"act_evid_run"`` (or the singleton kind if the run has length 1).
...
_RUN_KINDS: frozenset[EventKind] = frozenset({EventKind.ACT, EventKind.EVID})
