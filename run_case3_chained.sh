#!/usr/bin/env bash
# One-shot launcher: chained_fork eval on the single case-3 fixture.
# GT = {search, profile}. New auditor_minimal.md prompt.
...
uv run --no-sync python contrib/scenarios/rca/eval/scripts/retag_fixed_set.py \
    --fixture contrib/scenarios/rca/eval/fixtures/ops-lite-case3-only.txt \
    --dataset ops-lite \
    --tag ops-lite-case3-only

# 2. Render config with substituted env + remap tag.
out=$(mktemp /tmp/opslite-case3-XXXXXX.yaml)
trap 'rm -f "$out"' EXIT
envsubst < contrib/scenarios/rca/eval/config.ops-lite-fixed-50.yaml \
  | sed 's/ops-lite-fixed-50/ops-lite-case3-only/g' > "$out"

exp_id="agentm-chained-case3-promptv2-$(date +%H%M)"
echo
