...
The filesystem helpers and metadata record types (previously
``agentm.core.lib.artifact_files``) are inlined here because the current
branch has no ``ProjectLayout``; the artifacts directory is resolved
directly under ``<cwd>/.agentm/artifacts/<root_session_id>``.
"""
...
# Service key for the shared artifact store (previously
# ``agentm.core.abi.roles.ARTIFACT_STORE_SERVICE``, absent on this branch).
...
# === Artifact metadata + filesystem helpers (inlined) ======================
...
def scan_artifact_metadata(artifacts_dir: Path) -> list[ArtifactMetadata]:
    ...
    for meta_path in sorted(artifacts_dir.glob("*.meta.json")):
        try:
            ...
            logger.warning(
                "artifact_store: skipping corrupt metadata {}: {}", meta_path, exc
            )
            continue
...
class _StoreContext:
    ...
    def artifacts_dir(self) -> Path:
        # TODO(migration): the current branch has no ProjectLayout; artifacts
        # live under ``<cwd>/.agentm/artifacts/<root_session_id>`` instead of a
        # layout-resolved root.
        return self.base_dir / ".agentm" / "artifacts" / self.root_session_id


class ArtifactStore:
    def __init__(self, api: AtomAPI, config: ArtifactStoreConfig) -> None:
        self._api = api
        self._id_lock = asyncio.Lock()
        root_session_id = str(config.root_session_id or api.ctx.session_id)
    ...
    async def on_session_ready(self, event: SessionReadyEvent) -> None:
        ...
        self._ctx.task_id = getattr(event, "task_id", None)
        self._ctx.persona = getattr(event, "persona", None)
...
def install(api: AtomAPI, config: ArtifactStoreConfig) -> None:
