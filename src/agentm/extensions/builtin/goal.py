...
_CHECKER_PROMPT_TEMPLATE: Final[str] = (
    ...
    "The task's requirements are the sole authority — not the repository's "
    "current state. Judge the work against what the task asks for, never "
    "against artifacts the task is changing. An in-repo test or code path "
    "that encodes the old behavior is stale: its failure under a correct "
    "change is not grounds for rejection, and making it pass again is not "
    "evidence of correctness. Weigh every piece of evidence by whether it "
    "demonstrates the requirement, not by whether pre-existing checks "
    "stayed green.\n\n"
    ...
    "Ground your evidentiary demands in what this environment can do. If a "
    "validation avenue cannot run here — a tool that will not install, a "
    "command the sandbox cannot execute — its absence is not grounds for "
    "rejection; judge the alternative evidence the agent produced for the "
    "same behavior. Never withhold approval for want of a mechanism the "
...
_AUTO_INIT_PROMPT: Final[str] = (
    ...
    "the files, commands, and procedures it references both exist and can "
    "actually run here. Use cheap checks (list directories, glob for names, "
    "a quick invocation to confirm a validation tool starts rather than "
    "erroring on setup) — do NOT read source files end-to-end; grounding is "
    "about what the environment can do, not implementation detail. A "
    "condition that demands something this environment cannot provide or "
    "cannot run — a tool that will not install, a command the sandbox "
    "cannot execute — is worse than useless; it sends the agent chasing the "
    "impossible. Prefer verification the agent can perform directly; when "
    "the spec's stated procedure conflicts with what actually works here, "
    "derive the condition from what can be verified here and note the "
    "substitution.\n\n"
    ...
    "How the goal should be verified. Verification is grounded in the "
    "requirement, not in whatever tests happen to exist. For each part of "
    "the goal, derive the observable evidence that would show it satisfied, "
    "and expect that evidence from checks authored to exercise the "
    "requirement directly. Existing tests, scripts, or specs are inputs to "
    "consider, never the definition of done. Quote the passages of the "
    "source material that describe how the result should be validated, then "
    "state what the agent should author or run to produce the evidence — "
    "including tests it writes itself where the environment provides none. "
    ...
    "agree with the task's requirements — see the authority rule below.)\n"
    ...
    "IMPORTANT: the task's stated requirements are the authority. When "
    "existing code or tests disagree with what the task asks for, the task "
    "wins — never derive a backward-compatibility invariant from an "
    "artifact the task is asking to change.\n\n"
