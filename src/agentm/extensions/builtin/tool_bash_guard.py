...
files.  This atom intercepts ``bash`` tool calls and blocks known
...
def _on_tool_call(event: ToolCallEvent) -> dict[str, JsonValue] | None:
...
def install(api: AtomAPI, config: ToolBashGuardConfig) -> None:
    del config
