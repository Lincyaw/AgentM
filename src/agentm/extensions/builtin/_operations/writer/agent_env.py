"""ARL sandbox ``ResourceWriter`` implementation."""
...
class AgentEnvResourceWriter:
    ...
    def __init__(
        ...
        *,
        ...
        self._work_dir = normalize_work_dir(work_dir)
    ...
    def _resolve(self, path: str) -> str:
        return sandbox_abs(self._work_dir, path)

    def _relative(self, path: str) -> str | None:
        return workspace_relative_path(self._work_dir, path)

    def _in_sandbox(self, path: str) -> bool:
        return is_in_work_dir(self._work_dir, path)
    ...
    async def _write_bytes(self, rel_path: str, content: bytes) -> tuple[bool, str]:
        try:
            await call_maybe_async(self._session.upload_file, rel_path, content)
    ...
    async def read(self, path: str) -> bytes:
        ...
        if rel_path is not None:
            try:
                return await call_maybe_async(self._session.download_file, rel_path)
            except Exception as exc:
