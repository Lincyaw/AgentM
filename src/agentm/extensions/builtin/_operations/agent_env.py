"""ARL agent-env sandbox: config, shared utilities, session lifecycle.
...
Layout:
- ``bash/agent_env``  — ``BashOperations`` impl
- ``writer/agent_env`` — ``ResourceWriter`` impl

Both import shared ARL/path helpers from this module.
"""
...
def sandbox_abs(work_dir: str, path: str) -> str:
    if path.startswith("/"):
        return posixpath.normpath(path)
    return posixpath.normpath(posixpath.join(work_dir, path))


def is_in_work_dir(work_dir: str, path: str) -> bool:
    abs_path = sandbox_abs(work_dir, path)
    if work_dir == "/":
        return abs_path.startswith("/")
    return abs_path == work_dir or abs_path.startswith(work_dir + "/")


def workspace_relative_path(work_dir: str, path: str) -> str | None:
    abs_path = sandbox_abs(work_dir, path)
    if work_dir == "/":
...
def gateway_client_class(arl_module: Any) -> Any:
    return getattr(arl_module, "GatewayClient", None) or getattr(
        arl_module, "AsyncGatewayClient"
    )


def arl_class(arl_module: Any, *names: str) -> Any:
    for name in names:
...
def step_timeout_budget(steps: list[dict[str, Any]]) -> float | None:
    values: list[float] = []
    for step in steps:
...
async def async_execute(
    ...
    *,
    ...
    try:
        return await call_maybe_async(session.execute, steps, on_output=on_output)
...
async def _get_execute_operation(
    ...
    *,
    ...
    if getter is not None:
        return await call_maybe_async(getter, operation_id)
    ...
    client = gateway_client_class(arl)(base_url=gateway_url, api_key=api_key)
    try:
...
async def _recover_execute_operation(
    ...
    *,
    ...
    budget = step_timeout_budget(steps)
...
async def _replay_fork_environment(
    ...
    if not isinstance(source_arl_session, str) or not source_arl_session:
        ...
        client = gateway_client_class(arl)(base_url=gateway_url, api_key=api_key)
        try:
            arl_sessions = await call_maybe_async(
...
async def install_agent_env(api: ExtensionAPI, config: AgentEnvConfig) -> None:
    ...
    from .bash.agent_env import AgentEnvBashOperations
    from .writer.agent_env import AgentEnvResourceWriter
    ...
    if config.attach_session:
        sandbox_cls = arl_class(arl, "SandboxSession", "AsyncSandboxSession")
        session = await call_maybe_async(
            sandbox_cls.attach,
        ...
        managed_cls = arl_class(arl, "ManagedSession", "AsyncManagedSession")
        session = managed_cls(
            image=config.image,
    ...
    if owned:
        await call_maybe_async(session.create_sandbox)
