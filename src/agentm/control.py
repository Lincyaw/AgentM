...
async def send_interrupt(
    ...
    *,
    ...
    try:
        ...
        writer.write_eof()
        await writer.drain()
        response = (await reader.readline()).decode("utf-8", errors="replace").strip()
    finally:
        writer.close()
...
class SessionControlServer:
    """Expose interrupt-and-feedback over a local owner-only Unix socket."""
    ...
    async def _handle_connection(
        ...
        response = b"ok\n"
        try:
            data = await reader.read(_MAX_MESSAGE_BYTES + 1)
            ...
            logger.warning("session interrupt delivery failed: {}", exc)
