...
def to_jsonable(obj: Any, *, _depth: int = 0) -> Any:
    ...
    if is_dataclass(obj) and not isinstance(obj, type):
        return {
            field.name: to_jsonable(getattr(obj, field.name), _depth=_depth + 1)
            for field in fields(obj)
        }
    if isinstance(obj, dict):
        return {
            str(key): to_jsonable(value, _depth=_depth + 1)
            for key, value in obj.items()
        }
    if isinstance(obj, (list, tuple, set, frozenset)):
        return [to_jsonable(value, _depth=_depth + 1) for value in obj]
    obj_dict = getattr(obj, "__dict__", None)
    if isinstance(obj_dict, dict):
        return to_jsonable(obj_dict, _depth=_depth + 1)
    return repr(obj)
