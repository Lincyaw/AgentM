...
class LoopConfig:
    ...
    ``tool_execution="parallel"`` is reserved for a future revision; the
    kernel seed only honors ``"sequential"``. Passing ``"parallel"`` raises
    ``NotImplementedError`` at ``AgentLoop.run`` entry rather than silently
    serializing — surprises here would mask real concurrency bugs later.
    """
...
class AgentLoop:
    ...
    async def run(
        ...
        *,
        ...
        if self._config.tool_execution == "parallel":
            raise NotImplementedError(
                "LoopConfig.tool_execution='parallel' is reserved for a "
                "future revision; the kernel seed only supports 'sequential'."
            )
        ...
        try:
            for turn_index in range(max_turns):
                ...
                # Execute tool calls.
                # NOTE: spec allows config.tool_execution="parallel", but the
                # kernel seed deliberately runs tools sequentially to avoid
                # concurrency bugs in v0. TODO: implement parallel execution
                # respecting per-tool sequential markers.
