...
from agentm_rca.sanitizer.code_sanitizer import (
    CodeSanitizer,
...
def _ctx(step: int = 5, max_steps: int = 10) -> SanitizerContext:
...
def test_code_sanitizer_routing_overrides_and_disable_controls() -> None:
    ...
    overridden = CodeSanitizer(severity_map={"C1": Severity.WARN}).check(
        "hypothesis_change", hs2, ps2, tr2, _ctx()
    )
